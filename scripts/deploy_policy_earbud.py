...
**r** stop and reset arm+hand to ``start_pose`` (only when ``deployment.no_start_pose`` is false
and ``teleop_config.yaml`` defines ``start_pose.arm_joints``).
"""
...
USE_CONFIG_NAME = "config/dp_earbud_config"

# Zarr dataset used to compute the mean start pose (arm EEF + hand angles)
DATASET_PATH = "data/pickplace_earbud/place/ryhand_place_earbud.zarr"
