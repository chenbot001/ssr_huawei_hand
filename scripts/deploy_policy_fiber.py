...
Start pose (arm EEF Cartesian + hand joint angles) is taken from the mean
first-frame values computed by ``get_mean_start_pose.py`` on the optical fiber dataset.

Usage (from project root):
  python scripts/deploy_policy_fiber.py
  python scripts/deploy_policy_fiber.py deployment.dry_run=true
  python scripts/deploy_policy_fiber.py deployment.checkpoint_path=/path/to/best.ckpt
"""
...
USE_CONFIG_NAME = "config/dp_opticalfiber_config"

# Zarr dataset used to compute the mean start pose (arm EEF + hand angles)
DATASET_PATH = "data/opticalfiber/ryhand_opticalfiber_dataset.zarr"

# If True, start DDIM sampling from zeros instead of random noise (deterministic).
ZERO_INIT_NOISE = False
...
def move_to_start_pose(ur_arm, hand_ctrl, arm_eef_pose: np.ndarray, hand_pose: np.ndarray,
                       ...
                       servo_gain: int = 300):
    """Move arm to EEF pose and hand to joint angles, both from dataset mean start pose."""
...
def main(cfg: OmegaConf):
    ...
    if not os.path.isfile(ckpt_path):
        ...
        print("  python scripts/deploy_policy_fiber.py deployment.checkpoint_path=/path/to/best.ckpt")
        sys.exit(1)
    ...
    if ZERO_INIT_NOISE:
        print("[deploy] Zero-init noise enabled — overriding conditional_sample")
        _orig_cond_sample = policy.conditional_sample
        def _zero_cond_sample(condition_data, condition_mask, **kwargs):
            # trajectory = torch.zeros_like(condition_data)
            trajectory = torch.full_like(condition_data, -0.5)
            ...
            for t in scheduler.timesteps:
                ...
                model_output = policy.model(
                    trajectory, t,
                    local_cond=kwargs.get("local_cond"),
                ...
                trajectory = scheduler.step(
                    ...
                    **{k: v for k, v in kwargs.items()
                       if k not in ("local_cond", "global_cond", "generator")}
                ).prev_sample
            trajectory[condition_mask] = condition_data[condition_mask]
            return trajectory
        policy.conditional_sample = _zero_cond_sample
    ...
    # Compute mean start pose from dataset — overrides any hardcoded values
    ...
    try:
        while True:
            ...
            cv2.imshow("Deploy (Fiber)", canvas)
            ...
            if running:
                ...
                for i in range(steps_pi):
                    ...
                    hand_j[0] = np.deg2rad(10)   # thumb side_swing locked to +10°
                    hand_j[3] = 0.0               # index side_swing locked to 0
                    hand_j[6:15] = 0.0            # middle, ring, pinky fully disabled
